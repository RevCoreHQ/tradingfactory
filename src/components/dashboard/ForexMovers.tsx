...
/** Check if a Polygon ticker matches one of our tracked instruments */
function isTracked(ticker: string): boolean {
  const raw = ticker.replace("C:", "").replace("X:", "");
  return INSTRUMENTS.some(
    (i) => i.baseCurrency + (i.quoteCurrency || "") === raw ||
           raw === i.id.replace("_", "")
  );
}
  ...
  return (
    <div className="flex items-center gap-2 py-2 overflow-x-auto scrollbar-hide">
      ...
      <div className="flex items-center gap-1.5">
        {topGainers.map((m) => {
          const tracked = isTracked(m.ticker);
          return (
            <div
              key={m.ticker}
              className={cn(
                "flex items-center gap-1 px-2 py-1 rounded-md text-[11px] font-mono border shrink-0",
                tracked
                  ? "border-bullish/25 bg-bullish/8"
                  : "border-border/40 bg-[var(--surface-2)]"
              )}
            >
              <TrendingUp className="h-2.5 w-2.5 text-bullish" />
              <span className={cn("font-medium", tracked ? "text-foreground" : "text-muted-foreground")}>
          ...
          return (
            <div
              ...
              className={cn(
                "flex items-center gap-1 px-2 py-1 rounded-md text-[11px] font-mono border shrink-0",
                tracked
                  ? "border-bearish/25 bg-bearish/8"
                  : "border-border/40 bg-[var(--surface-2)]"
              )}
            >
