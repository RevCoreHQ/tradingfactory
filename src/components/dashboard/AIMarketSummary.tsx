...
export function AIMarketSummary() {
  ...
  const focusSecondary = summary.focusTodaySecondary ?? [];
  ...
  return (
    <div className="relative section-card p-3 sm:p-5">
      ...
      {(focusToday.length > 0 || focusSecondary.length > 0 || sitOutToday.length > 0) && (
        <div className="mb-4 space-y-3">
            ...
            {focusToday.length > 0 && (
              <div className="rounded-lg border border-bullish/20 bg-bullish/5 px-3 py-2.5">
                ...
                <p className="text-[10px] text-muted-foreground/80 mb-1.5 leading-snug">
                  Clearer desk edge (consider / lean on instrument cards)
                </p>
          ...
          {focusSecondary.length > 0 && (
            <div className="rounded-lg border border-[var(--amber)]/25 bg-[var(--amber)]/[0.06] px-3 py-2.5">
              <div className="flex items-center gap-1.5 mb-1.5">
              ...
              <p className="text-[10px] text-muted-foreground/80 mb-1.5 leading-snug">
                Still thematic, but instrument cards show wait or no-trade filter — do not size like primary focus.
              </p>
              <div className="flex flex-wrap gap-1.5">
                {focusSecondary.map((f) => (
                  <span
                    key={f}
                    className="text-[11px] font-mono px-2 py-0.5 rounded-md bg-background/50 text-[var(--amber)] border border-[var(--amber)]/20"
                  >
                    {f}
                  </span>
                ))}
        ...
        {summary.opportunities.length > 0 && (
          <div className="space-y-1.5">
            <div className="flex flex-col gap-0.5 sm:flex-row sm:items-start sm:gap-2">
              ...
              <span className="text-[10px] text-muted-foreground/70 leading-snug sm:pt-0.5">
                Carry, USD regime, rates — not a second ticker list (duplicates removed vs Focus)
              </span>
            </div>
